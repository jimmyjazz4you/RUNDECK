...
public class S3LogFileStoragePlugin implements ExecutionFileStoragePlugin, AWSCredentials, ExecutionMultiFileStorage {
            ...
            throws ExecutionFileStorageException
    {
        ...
        logger.log(Level.SEVERE, "getState for S3 bucket {0}:{1}", new Object[]{getBucket(),
                key});
        try {
            ...
            logger.log(Level.SEVERE, "Metadata {0}", new Object[]{userMetadata});
            ...
            if (null != expectedMeta) {
                for (String s : expectedMeta.keySet()) {
                    ...
                    if (!matches) {
                        logger.log(Level.SEVERE, "S3 Object metadata '{0}' was not expected: {1}, expected {2}",
            ...
            if (e.getStatusCode() == 404) {
                //not found
                logger.log(Level.SEVERE, "getState: S3 Object not found for {0}", key);
            } else {
    ...
    protected boolean storePath(
            ...
            throws ExecutionFileStorageException
    {
        logger.log(Level.SEVERE, "Storing content to S3 bucket {0} path {1}", new Object[]{getBucket(), key});
        InputStreamReader reader = new InputStreamReader(stream);
        try (BufferedReader bufferedReader 
	= new BufferedReader(new InputStreamReader(stream))) { 
        logger.log(Level.SEVERE, "Puede que el stream sea nulo {0}", stream.available());
        logger.log(Level.SEVERE, "Puede que el stream 1 sea nulo {0}", reader.ready());
	String line; 
	while ((line = bufferedReader.readLine()) != null) { 
        ...
        try {
            ...
            logger.log(Level.SEVERE, "Job could still be executing", e.getMessage());
            logger.log(Level.SEVERE, e.getMessage());
            logger.log(Level.SEVERE, e.getMessage(), e);
            throw new ExecutionFileStorageException(e.getMessage(), e);
        } catch (AmazonClientException e) {
