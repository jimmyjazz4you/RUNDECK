...
public class S3CryptoModuleAE extends S3CryptoModuleBase<MultipartUploadCryptoContext> {
      ...
      if (wrapped.hasEncryptionInfo()) {
         ...
         if (ifile != null) {
            ...
            throw new SecurityException(
               "Unencrypted object found, cannot be decrypted in mode "
         ...
         } else if (this.cryptoConfig.isUnsafeUndecryptableObjectPassthrough()) {
            this.log
               .warn(
                  String.format(
   ...
   private S3Object decipherWithInstructionFile(
      ...
      ContentCryptoMaterial cekMaterial = ContentCryptoMaterial.fromInstructionFile(
         matdesc, this.kekMaterialsProvider, this.cryptoConfig, cryptoRange, extraMatDesc, keyWrapExpected, this.kms
      );
   ...
   private S3Object decipherWithMetadata(GetObjectRequest req, long[] desiredRange, long[] cryptoRange, S3ObjectWrapper retrieved) {
      ...
      ContentCryptoMaterial cekMaterial = ContentCryptoMaterial.fromObjectMetadata(
         retrieved.getObjectMetadata().getUserMetadata(), this.kekMaterialsProvider, this.cryptoConfig, cryptoRange, extraMatDesc, keyWrapExpected, this.kms
      );
      boolean isRangeGet = desiredRange != null;
      this.securityCheck(cekMaterial, retrieved.getS3ObjectId(), isRangeGet);
      S3ObjectWrapper decrypted = this.decrypt(retrieved, cekMaterial, cryptoRange);
